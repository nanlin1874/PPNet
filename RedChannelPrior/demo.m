...
filename = dir(fullfile(path,'*.jpg'));
b = length(filename);

%save image
save_dirname = 'E:\AutomaticRedChannelRestoration\AutomaticRedChannelRestoration\rdcp';
n = 'md';
savedir = strcat(n,{32}, save_dirname);
dos(char(savedir));

for j =1:b
    fname = strcat(path, '/', filename(j).name);
    img =imread(fname);
...
r = krnlsz;
eps = 10^-6;
tran = guidedfilter(double(rgb2gray(img))/255, t, r, eps);
...
J = cat(3,Jr,Jg,Jb);
J = uint8(J * 255);
imwrite(t,fullfile(save_dirname,filename(j).name));
