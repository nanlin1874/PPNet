...
flags.DEFINE_integer("c_ctran_dim", 1, "Dimension of ctran. [1]")
...
def main(_):
  ...
  data_dir1 = os.path.join(os.getcwd(), 'rcp')
  ...
  data_dir2 = os.path.join(os.getcwd(), 'cp')
  data2 = sorted(glob.glob(os.path.join(data_dir2, "*.png")))
  test_data_list2 = data2 + sorted(glob.glob(os.path.join(data_dir2, "*.jpg"))) + sorted(
      glob.glob(os.path.join(data_dir2, "*.bmp"))) + sorted(glob.glob(os.path.join(data_dir2, "*.jpeg")))
  ...
  for ide in range(0,len(test_data_list)):
    ...
    ctran_test1 = get_image(test_data_list2[ide], is_grayscale=False)
    Ctran = Image.fromarray(np.uint8(ctran_test1 * 255))
